...
export const createInvoiceController = CatchError(async (req: Request & { user?: any }, res: Response) => {
  const { productId, userEmail, userPhone, amount, type } = req.body
  if (!productId || !userEmail || !userPhone || !amount || !type) {
...
export const approveInvoiceController = CatchError(async (req: Request & { user?: any }, res: Response) => {
  ...
  await session.withTransaction(async () => {
    ...
    const computedStatus = existing.type === 'rent' ? 'ACTIVE' : 'PAID'
    ...
    if (productId) {
      const nextProductStatus = invoice.type === 'rent' ? 'RENTED' : 'ACTIVE'
      await updateProductStatus(productId, nextProductStatus, session)
    }
  })
  session.endSession()
