...
import { normalizeCommissionConfig } from '../../commission/service/commission-calculator'

function isValidRole(role: string): boolean {
  return Object.values(UserRole).includes(role as UserRole)
}

const getCommissionPayload = (input: { commissionRate?: unknown; commissionConfig?: unknown }) => {
  const fallbackRate = Number(input.commissionRate)
  const normalizedFallbackRate = Number.isFinite(fallbackRate) ? fallbackRate : 0
  const commissionConfig = normalizeCommissionConfig(input.commissionConfig, normalizedFallbackRate)
  return {
    commissionRate: commissionConfig.defaultRate,
    commissionConfig,
  }
}
...
export const registerUser = CatchError(async (req: Request, res: Response) => {
  try {
    ...
    const { creditLimit, walletBalance, commissionRate, commissionConfig } = req?.body
    if (!isValidRole(role)) {
      return res.status(400).json(new ApiResponse(400, { message: 'Invalid role' }, 'Invalid role'))
    }
    const commissionPayload = getCommissionPayload({ commissionRate, commissionConfig })
    ...
    const payload = {
      ...
      creditLimit,
      walletBalance,
      commissionRate: commissionPayload.commissionRate,
      commissionConfig: commissionPayload.commissionConfig,
    }
...
export const updateUserByEmailForAdmin = CatchError(async (req: Request, res: Response) => {
  ...
  const hasCommissionConfig = req.body?.commissionConfig !== undefined
  const commissionPayload = hasCommissionRate || hasCommissionConfig ? getCommissionPayload(req.body || {}) : null
