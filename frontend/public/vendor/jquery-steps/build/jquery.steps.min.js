!(function (a, b) {
  ...
  function LG(a, b, c) {
    if (b.enablePagination) {
      a.find('.actions.finish-btn').remove();

      var d = '<{0} class="actions finish-btn {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
        f = '';
      if (!document.getElementById(`form-${c.stepCount}`) && c.currentIndex + 1 == c.stepCount) {
        f += `<li><button type="submit" id="form-${c.stepCount}" class="btn btn-primary font-weight-bold">${b.labels.finish}</button></li>`;
      }
